...
namespace MindBoxTask
{
    ...
    public class Triangle : Shape
    {
        ...
        {
            if (ab < 0 || bc < 0 || ac < 0) throw new ArgumentException($"Tакого треугольника не существует(сторона(ы) меньше 0)");
            else if (ab > (bc + ac) || bc > (ab + ac) || ac > (ab + bc)) throw new ArgumentException($"Такого треугольника не существует(сторона больше суммы двух других)");
            else
            {
                this.ab = ab;
                this.bc = bc;
